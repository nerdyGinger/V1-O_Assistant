...
import time, uuid, urllib, urllib.request, hmac, hashlib
...
def yahooQuery(query):    
    oauth = { 'oauth_consumer_key': YAHOO_CLIENT_ID,
    ...
    signBaseStr = bytes('GET&' + quote(BASE_URL, safe='') + '&'
                  + quote('&'.join(paramsSort), safe=''), 'utf-8')
    compositeKey = bytes(quote(YAHOO_CLIENT_SECRET, safe='') + '&', 'utf-8')
